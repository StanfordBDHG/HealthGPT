...
class PromptGenerator {
    ...
    private func buildFourteenDaysHealthDataPrompt() -> String {
        ...
        for day in 0...13 {
            let dayData = healthData.dailyActivityData[day]
    ...
    private func buildOneDayHealthDataPrompt(with dayData: ActivityData) -> String {
        var dayPrompt = ""
    ...
    private func buildEhrDataPrompt() -> String {
        var ehrPrompt = "The following list contains all patients EHR records as JSON objects: "
        let ehrDataList = healthData.ehrData ?? []
        ehrPrompt += "[ \(ehrDataList.joined(separator: ",")) ]"

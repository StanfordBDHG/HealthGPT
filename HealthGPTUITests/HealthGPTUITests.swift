...
extension XCUIApplication {
    ...
    private func navigateOnboardingFlowWelcome() throws {
        XCTAssertTrue(staticTexts["HealthGPT"].waitForExistence(timeout: 5))

        XCTAssertTrue(buttons["Continue"].waitForExistence(timeout: 5))
        buttons["Continue"].tap()
    }

    private func navigateOnboardingFlowDisclaimer() throws {
        XCTAssertTrue(staticTexts["Disclaimer"].waitForExistence(timeout: 5))
        ...
        XCTAssertTrue(buttons["I Agree"].waitForExistence(timeout: 5))
        buttons["I Agree"].tap()
    }

    private func navigateOnboardingFlowApiKey() throws {
        XCTAssertTrue(staticTexts["OpenAI API Key"].waitForExistence(timeout: 5))
        XCTAssertTrue(buttons["Save API Key"].waitForExistence(timeout: 5))
    ...
    private func navigateOnboardingFlowModelSelection() throws {
        XCTAssertTrue(staticTexts["Select an OpenAI Model"].waitForExistence(timeout: 5))
        XCTAssertTrue(buttons["Save"].waitForExistence(timeout: 5))
